...
def load_data():
    ...
    train_imgs = os.listdir("./car_logo_finail/Car logo/trainImg")
    num_train_imgs = len(train_imgs)
    for i in range(num_train_imgs):
        img = Image.open("./car_logo_finail/Car logo/trainImg/" + train_imgs[i])
        arr = np.asarray(img)
        train_data[i,:,:,:] = [arr[:,:,0], arr[:,:,1], arr[:,:,2]]
        train_labels[i] = int(train_imgs[i].split('.')[0])

    test_imgs = os.listdir("./car_logo_finail/Car logo/testImg")
    num_test_imgs = len(test_imgs) 
    for i in range(num_test_imgs):
        img = Image.open("./car_logo_finail/Car logo/testImg/" + test_imgs[i])
        arr = np.asarray(img)
